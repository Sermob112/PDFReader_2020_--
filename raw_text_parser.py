...
def clean_text_file(input_path, output_path):
    ...
    start_patterns = [
        r"Total number of sister ships still",
        r"IMO number",
    ...
    for i, line in enumerate(lines):
        ...
        if re.match(r'^[\d\$%№\\&\t\s]+', line.strip()):
            ...
            if merged_line:
                ...
                merged_line = ""
            continue
        if "Significant Ships" in line or "Significant ShipS" in line:
            continue
        if merged_line:
...
input = "Raw_text\SS 2007.txt"
output= "Processed_text\Proccesed_SS2007.txt"
